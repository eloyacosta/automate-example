#Datarobot test by eacosta@gmail.com
		...
		for elem in my_json:
			if type(elem) == dict:
				...
				if val_element is False:
					return jsonify(success='false', message='Invalid checksum or field type for element %s' % json.dumps(elem))
				...
				return jsonify(success='false', message='Invalid elements in the list: %s' % json.dumps(elem))
	elif type(my_json) == dict:
		#I sould be receiving an single element
		val_element=validate(my_json)
		if val_element is False:
			return jsonify(success='false', message='Invalid checksum or field type for element %s' % json.dumps(my_json)) 
		#Overwirte element without checksum as I don't want to store it, and with a parsed date format
